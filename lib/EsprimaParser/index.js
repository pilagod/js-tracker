...
class EsprimaParser {
  ...
  initAssignmentOperators(importedAssignmentOperators = {}) {
    return Object.assign({}, importedAssignmentOperators, {
      '=': ({object, property, info}, value) => {
        // @TODO: dom property assign, ex: style, innerHTML etc. (dispatch object and property)

        // @TODO: doAssignOperation
        if (object === undefined) {
  ...
  checkAndExecute(data, info) {
    const calleeStack = this.createCalleeStack(data.splice(0, 1))
    const checkAndExecuteReducer = this.createCheckAndExecuteReducer(info)
    // reduce on data[1] to data[length - 1]
    data.reduce(checkAndExecuteReducer, calleeStack)

    return this.getLastElement(calleeStack)
  }

  createCalleeStack(data) {
  ...
  checkAndExecuteReducer(info, calleeStack, expression) {
    // expression might be a property string or a Method instance
    const callee = this.getLastElement(calleeStack)
    const status = this.callChecker.dispatch(callee, expression)

    this.addAffectedElementToCollection(info, callee, expression, status)
    this.updateCalleeStack(calleeStack, callee, expression, status)

    return calleeStack
  }

  addAffectedElementToCollection(info, callee, expression, status) {
    // @TODO: getAffectedElement
    // @TODO: addElementToCollection

    // if (status) {
    //   const affectedElement = this.getAffectedElement(callee, expression, status)
    //
    //   this.addElementToCollection(info, affectedElement)
    // }
  }

  getAffectedElement(callee, expression, status) {
    // @TODO: curStatus.hasOwnProperty('passive') ? curExp.arguments[curStatus.passive] : curResult
    // @TODO: manipulation do nothing (prepend, should get right target)
  }

  addElementToCollection(info, affectedElement) {
    // @TODO: addEvent
    // @TODO: addManipulation
    // @TODO: collection key should combine with loc
  }

  updateCalleeStack(calleeStack, callee, expression, status) {
    // @TODO: pop
...
// @TODO: Object.assign(EsprimaParser.prototype, statusHandlers)
