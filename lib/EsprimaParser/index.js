...
class EsprimaParser {
  ...
  VariableDeclarator(variableDeclarator) {
    ...
    const values = this.getInitValues(variableDeclarator.init)
  ...
  getUpdateValue(updateExpression) {
    const assignmentExpression = this.transUpdateToAssignment(updateExpression)

    return this.AssignmentExpression(assignmentExpression)
  }

  transUpdateToAssignment(updateExpression) {
    return {
  ...
  AssignmentExpression(assignmentExpression) {
    // {caller, callee}
    const exp = this.getRefExp(assignmentExpression.left)
