...
class EsprimaParser {
  ...
  FunctionExpression(functionExpression) {
    // @TODO: set prototype __proto__ given extends object
  ...
  parseFunctionAgent(functionAgent, context, calledArguments) {
    ...
    this.setFunctionEnvironment(functionAgent)
    this.setFunctionContext(context)
    this.setFunctionArguments(functionAgent.params, calledArguments)

    const result = this.parseNode(functionAgent.body)

    this.resetEnvironment(environment)
  ...
  getEnvironment() {
    return {
      scriptUrl: this.scriptUrl,
      closureStack: this.closureStack
    }
  }

  setFunctionEnvironment(functionAgent) {
    this.scriptUrl = functionAgent.scriptUrl
    this.closureStack = functionAgent.closureStack.getStack()
  ...
  setFunctionArguments(params, calledArguments) {
    for (let i = 0; i < params.length; i += 1) {
      this.setVariables(params[i], calledArguments[i])
    }
  }

  resetEnvironment(environment) {
    this.scriptUrl = environment.scriptUrl
    this.closureStack.popClosure()
    this.closureStack = environment.closureStack
  }
