...
class EsprimaParser {
  ...
  addInfoToCollection(callee, expression, info, status) {
    const affectedElements =
      this.getAffectedElements(callee, expression, status)

    this.addInfoToElements(affectedElements, info, status)
  }

  getAffectedElements(callee, expression, status) {
    const affectedObject =
      this.getAfftectedObject(callee, expression, status)

    return this.getElementsFromAffectedObject(affectedObject)
  }

  getAfftectedObject(callee, expression, status) {
    if (status.hasOwnProperty('passive')) {
      return expression.arguments[status.passive]
    } else if (this.isStyleOrDOMTokenList(callee)) {
