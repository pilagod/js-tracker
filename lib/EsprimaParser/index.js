...
class EsprimaParser {
  ...
  BlockStatement(blockStatement) {
    ...
    const result = this.parseBlockStatementBody(blockStatement.body)
    // @TODO: pop block scope from closureStack
    return result
  }

  parseBlockStatementBody(body) {
    let result
  ...
  parseSwitchCases(switchCases, discriminant) {
    ...
    for (const [matchedIndex, switchCase] of switchCases.entries()) {
      const matched = this.isCaseTestMatchDiscriminant(
        switchCases.test,
        discriminant
      )
      if (matched) {
        result = this.parseMatchedSwitchCase(switchCases, matchedIndex)
        break
      }
    }
    return result
  ...
  isCaseTestMatchDiscriminant(caseTestExpression, discriminant) {
    return (
      this.parseNode(caseTestExpression) === discriminant
    )
  }

  parseMatchedSwitchCase(switchCases, matchedIndex) {
    const firstNonEmptyCaseIndex = this.getFirstNonEmptyCaseIndex(switchCases, matchedIndex)

    return this.parseSwitchCasesUntilBreakOrEnd(
      switchCases,
  ...
  getFirstNonEmptyCaseIndex(switchCases, matchedIndex) {
    // @TODO: should get first not empty case consequent
    
  }

  parseSwitchCasesUntilBreakOrEnd(switchCases, firstNonEmptyCaseIndex) {
