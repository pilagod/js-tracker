...
/* import structures */
const CalleeAgent = require('./structures/CalleeAgent')
...
class EsprimaParser {
  constructor() {
    ...
    /* import structures */
    this.CalleeAgent = CalleeAgent // main use for execute
  ...
  createAddEventListenerFromPropEvent(propEvent, handler) {
    const addEventListener = new (this.CalleeAgent)('addEventListener')
  ...
  parseCallee(calleeExpression) {
    ...
    // this.getMethodInstance(callee) here ?
  }

  parseMemberCallee(calleeExpression) {
    const caller = this.getObjectAsExpressionArray(calleeExpression.object)
    const callee = this.getPropertyKeyAsString(
      calleeExpression.property,
      calleeExpression.computed
    )
    return {
      caller,
      callee: this.getCalleeAgent(callee)
    }
  }

  getCalleeAgent(callee) {
    return new (this.CalleeAgent)(callee)
  }

  parseOtherCallee(calleeExpression) {
    const callee = this.parseNode(calleeExpression)
  ...
  executeReducer(pre, cur) {
    return (cur instanceof this.CalleeAgent) ?
  ...
  checkAndExecuteReducer(info, caller, callee) {
    ...
    const status = this.callChecker.dispatch(caller, callee)
