...
class EsprimaParser {
  ...
  getLoopStatusAndReset() {
    if (this.flowStatus.isLoopBreakStatus()) {
      ...
      this.flowStatus.unset('continue')
      return 'continue'
    }
    return null
  }
  ...
  FunctionExpression(functionExpression) {
    ...
    return new (this.FunctionAgent)({
      ...
      parser: this.parseFunction.bind(this),
      scriptUrl: this.scriptUrl,
      closureStack: this.closureStack.getCurrentStack()
    })
  }
  ...
  handleReferenceOperation(argument, operation, ...args) {
    switch (argument.type) {
      case 'MemberExpression': {
        ...
        const expressionReference = this.getExpressionReference(expression)

        return operation(expressionReference, ...args)
      }
      default:
        return operation({
          property: this.getNameFromPattern(argument)
        }, ...args)
    }
  }

  getExpressionReference(expression) {
    // @TODO: instance of style / classList should keep its parent
    // @TODO: return {
    //   object
    //   property
    //   info
  ...
  executeExpression(data) {
    ...
    this.checkStyleOrClassList(result)

    return result
  }

  checkStyleOrClassList(executedExpression) {
    // @TODO: this.closureStack.get('window')
    // @TODO: check result is CSSStyleDeclaration /
    // @TODO: Element.classList、HTMLLinkElement.relList、HTMLAnchorElement.relList 或 HTMLAreaElement.relList
  }
