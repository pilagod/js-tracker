...
class EsprimaParser {
  ...
  isCaseMatched(caseTestExpression, discriminant) {
  ...
  parseMatchedCase(switchCases, matchedIndex) {
    ...
    // start parsing statements from firstNonEmptyCaseIndex,
    // if the case has no break / return statement,
    // it should keep parsing consequent cases until any break / return or end of default case
    return this.parseStatements(switchCases.slice(firstNonEmptyCaseIndex))
  }
  ...
  getFirstNonEmptyCaseIndex(switchCases, matchedIndex) {
    ...
    for (let i = matchedIndex; i < switchCasesCount; i += 1) {
      const nonEmptyOrDefault = this.isCaseNonEmptyOrDefault(switchCases[i])

      if (nonEmptyOrDefault) {
  ...
  isCaseNonEmptyOrDefault(switchCase) {
  ...
  SequenceExpression(sequenceExpression) {
    ...
    sequenceExpression.expressions.forEach((expression) => {
      // all are expressions, no need to check flow control status
