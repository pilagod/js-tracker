...
class EsprimaParser {
  ...
  ForInStatement(forInStatement) {
    const left = this.getName(forInStatement.left)
  ...
  getName(node) {
    switch (node.type) {
      case 'VariableDeclaration':
        this.parseNode(node) // do variable initialization
  ...
  getNameFromVariableDeclaration(node) {
    return this.getName(node.declarations[0].id)
  }
  ...
  VariableDeclarator(variableDeclarator) {
    ...
    const variables = this.getName(variableDeclarator.id)
