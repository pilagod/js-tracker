...
class EsprimaParser {
  ...
  initAssignmentOperators(importedAssignmentOperators = {}) {
    return Object.assign({}, importedAssignmentOperators, {
      '=': ({object, property, info}, value) => {
        ...
        // @TODO: event -> transform to addEventListener
        // @TODO: dispatch object and property
  ...
  checkAndExecuteReducer(info, callee, expression) {
    ...
    this.addInfoToCollection(callee, expression, status, info)

    return this.getNextCallee(callee, expression, status)
  }

  addInfoToCollection(callee, expression, status, info) {
    if (status) {
      const affectedElement =
        this.getAffectedElement(callee, expression, status)

      this.addInfoToElement(affectedElement, status, info)
    }
  }

  getAffectedElement(callee, expression, status) {
    return status.hasOwnProperty('passive') ?
      expression.arguments[status.passive] : callee
  }

  addInfoToElement(element, status, info) {
    switch (status.type) {
  ...
  getNextCallee(callee, expression, status) {
    return status.hasOwnProperty('execute') ?
      status.execute : this.execute([callee, expression])
  }
