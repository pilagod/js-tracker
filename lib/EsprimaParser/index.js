...
class EsprimaParser {
  ...
  setFunctionContext(context = this.context) {
  ...
  addInfoToCollection(target, info, status) {
    const object = this.getTargetObject(target, status)
    const elements = this.getTargetElements(object)

    this.addInfoByStatus({elements, info}, status)
  }

  getTargetObject({caller, callee}, status) {
    // @TODO: if HTMLString
    // @TODO: if Attr
