...
const CallChecker = require('./structures/CallChecker')
...
class EsprimaParser {
    ...
    return Object.assign({}, importedUnaryOperators, {
      'delete': ({object, property}) => {
        ...
        if (targetObject === undefined) {
          targetObject = this.closureStack.getContext()
        }
  ...
  initAssignmentOperators(importedAssignmentOperators = {}) {
    return Object.assign({}, importedAssignmentOperators, {
      '=': ({object, property, info}, value) => {
        const status = this.callChecker.dispatch(object, property)
  ...
  handleAssignManipulation(object, property, info, value, status) {
    ...
    if (status.type === CallChecker.EVENT) {
  ...
  createAddEventListenerFromPropEvent(propEvent, handler) {
    const addEventListener = new (this.Method)('addEventListener')
    const mappedEvent = this.getEventFromPropEvent(propEvent)

    addEventListener.addArguments([mappedEvent, handler])
  ...
  parseCallExpression(callExpression) {
    ...
    method.addArguments(parsedArguments)
  ...
  executeReducer(pre, cur) {
    return (cur instanceof this.Method) ?
  ...
  isStyleOrDOMTokenList(executedData) {
    const context = this.closureStack.getContext()
  ...
  checkAndExecute(data, info) {
    const callee = this.execute(data.splice(0, 1))
    const checkAndExecuteReducer = this.createCheckAndExecuteReducer(info)
    // reduce on data[1] to data[length - 1]
    return data.reduce(checkAndExecuteReducer, callee)
  }

  createCheckAndExecuteReducer(info) {
    return (...args) => {
      return this.checkAndExecuteReducer(info, ...args)
    }
  }

  checkAndExecuteReducer(info, callee, expression) {
    const status = this.callChecker.dispatch(callee, expression)

    if (status) {
      this.addInfoToCollection(callee, expression, info, status)
    }
    return this.getNextCallee(callee, expression, status)
  }

  addInfoToCollection(callee, expression, info, status) {
    const affectedElement =
      this.getAffectedElement(callee, expression, status)

    this.addInfoToElement(affectedElement, info, status)
  }

  getAffectedElement(callee, expression, status) {
    if (status.hasOwnProperty('passive')) {
      return expression.arguments[status.passive]
    }
    return this.getElementFrom(callee)
  }

  getElementFrom(callee) {
    if (this.isStyleOrDOMTokenList(callee)) {
      return callee.parent
    }
    return callee
  }

  addInfoToElement(element, info, status) {
    // @TODO: check if jquery -> call .get()
    switch (status.type) {
      case CallChecker.EVENT:
        this.collection.addEvent(element, info)
        break

      case CallChecker.MANIPULATION:
        this.collection.addManipulation(element, info)
        break
  ...
  getNextCallee(callee, expression, status) {
    if (status.hasOwnProperty('execute')) {
      return status.execute
    }
    return this.execute([callee, expression])
  }
