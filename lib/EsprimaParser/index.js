...
class EsprimaParser {
  constructor() {
    ...
    this.binaryOperators = this.initBinaryOperators(binaryOperators)
    ...
    this.checkerDispatcher = new CheckerDispatcher()
  }
  ...
  initBinaryOperators(importedBinaryOperators = {}) {
    return Object.assign({}, importedBinaryOperators, {
      'instanceof': (left, right) => {
        if (left.constructor instanceof this.FunctionAgent) {
          // check if right prototype is in left prototype chain
          return this.isInPrototypeChain(left, right.prototype)
        }
  ...
  isInPrototypeChain(currentObject, targetPrototype) {
    // check if right is in left prototype chain
    const currentPrototype = Object.getPrototypeOf(currentObject)

    return this.isPrototypeMatched(
      currentPrototype,
      targetPrototype
    )
  }

  isPrototypeMatched(currentPrototype, targetPrototype) {
    if (currentPrototype === null) {
      return false
    } else if (currentPrototype === targetPrototype) {
      return true
    }
    return this.isInPrototypeChain(
      currentPrototype,
      targetPrototype
    )
  }
  ...
  FunctionExpression(functionExpression) {
    ...
    // functionAgent.prototype = Object.create(superClass)
  ...
  createFunctionAgent(functionExpression) {
    ...
    const functionAgentParser = this.getFunctionAgentParser()

    // @TODO: wrap with a function
    return new (this.FunctionAgent)(Object.assign(
      {parser: functionAgentParser},
      functionEnvironment,
      functionInfo
    ))
  }
  ...
  getFunctionAgentParser() {
    return this.parseFunctionAgent.bind(this)
  }
  ...
  getMethod(pre, cur) {
    // @TODO: custom constructor type should check prototype
  ...
  checkAndExecute(data, info) {
    const resultStack = this.createResultStack(data.splice(0, 1))
    const checkAndExecuteReducer = this.createCheckAndExecuteReducer(info)
    // reduce on data[1] to data[length - 1]
    data.reduce(checkAndExecuteReducer, resultStack)

    return this.getLastElementOfStack(resultStack)
  }

  createResultStack(data) {
  ...
  checkAndExecuteReducer(info, resultStack, curExp) {
    const curResult = this.getLastElementOfStack(resultStack)
    // @TODO: StatusChecker.dispatch
    const curExpStatus = this.checkerDispatcher.dispatch(curResult, curExp)

    this.handleCurExpStatus(info, resultStack, curExpStatus)

    return resultStack
  }

  handleCurExpStatus(info, resultStack, curExpStatus) {
    // @TODO: should know curExpStatus type
    // @TODO: manipulation do nothing (prepend, should get right target)
    // @TODO: event should wrap handler
    // @TODO: ajax should wrap success / error

    // @TODO: native return undefined
    // @TODO: jQuery return $

    // @TODO: check curExpStatus.state type

    // @TODO: addEvent (should traverse each one to register own handler)
    // @TODO: addManipulation
    // @TODO: pop

    // @TODO: an array of each iterate result, get new value by executing last one with current expression
    // @TODO: pop -> pop result array
  }

  getLastElementOfStack(stack) {
